...
<body>
    ...
    @if (!empty($filters))
        <div class="filters">
            ...
            @if (!empty($filters['master_pemasukan_id']))
                @php
                    $masterPengeluaran = \App\Models\MasterPengeluaran::find($filters['master_pemasukan_id']);
                @endphp
                @if ($masterPengeluaran)
                    <p>Jenis Pemasukan: {{ $masterPengeluaran->nama_pengeluaran }}</p>
                @endif
            @endif
        </div>
    @endif
    ...
    <table>
        ...
        <tbody>
            @foreach ($transaksiKeluarData as $transaksi)
                <tr>
                    <td>{{ \Carbon\Carbon::parse($transaksi->tanggal_transaksi)->format('d-m-Y') }}</td>
                    <td>{{ $transaksi->user->name ?? '-' }}</td>
                    <td>{{ $transaksi->masterPengeluaran->nama_pengeluaran ?? '-' }}</td>
            ...
            <tr>
                ...
                <td style="font-weight: bold;">Rp {{ number_format($transaksiKeluarData->sum('jumlah'), 2, ',', '.') }}</td>
                <td colspan="2"></td>
            </tr>
        </tbody>
    </table>
